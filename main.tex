...
\noindent\hrulefill~\XU{[BEGIN]}~\hrulefill

【 自 17-08-17 13:50 】

The Epoch of Reionization (EoR) is a cosmological periodization referring to
a period of our universe preceded by the dark age. During EoR, which lasted
from about 150 million to 1 billion years after the Big Bang (i.e., at a
redshift $?? < z < ??$; ref??), reionization primarily caused by the $uv$
and soft X-ray photons from the first generation celestial objects surpassed
the cooling of the gas through recombination of electrons and protons. As a
result the majority of baryonic matter was again in highly ionized state.
...
reionization process, the detection the redshifted 21 cm emission from
neutral hydrogen, especially those from the adjacent regions of the ionized
bubbles, is recognized to be the decisive tool to directly reveal this epoch
(ref??).

In order to probe EoR a number of radio interferometric experiments working
in $\sim 100 - 200$ MHz have been designed to target the redshifted 21 cm
signals. These include the LOw Frequency ARray (LOFAR; Harker et al. 2010;
Jelic et al. 2008; Labropoulos et al. 2009; Yatawatta et al. 2013; van
Haarlem et al. 2013), 21 CentiMeter Array (21CMA; Zheng et al. 2012), the
Giant Meterwave Radio Telescope (GMRT; Pen et al. 2009; Paciga et al. 2010;
Paciga et al. 2013), the MIT Epoch of Reionization Experiment (MITEoR; Zheng
et al. 2014), the Donald C. Backer Precision Array for Probing the Epoch of
Reionization (PAPER; Backer et al. 2007; Parsons et al. 2010) the Murchison
Widefield Array (MWA; Mitchell et al. 2008; Lonsdale et al. 2009; Bowman et
al. 2006; Lonsdale et al. 2009; Tingay et al. 2013; Bowman et al. 2013), and
those to be built and operated in the near future, such as the Square
Kilometre Array (SKA; ref??) and the Hydrogen Epoch of Reionization Arrays
(HERA; ref??)
% 【每个实验均书写冠名全称，引用最原始的1-2篇文献】.

The challenges in these experiments, however, are particularly significant
due to a variety of complex instrumental effects (gain, chromatic point
spread function, polarization effects, sidelobe, noise, etc), ionospheric
distortions, radio frequency interference (RFI【如果只出现一次似无需缩写】),
and strong celestial foregrounds. For example, the magnitude of the
cumulative foreground sources, which is mainly contributed by the diffuse
polarized synchrotron radiation from the Milky Way and is complicated by
other components, such as the non-thermal continuum radiation of numerous
extragalactic radio sources, overwhelms the 21 cm signals by $4 - 5$ orders
(ref??), indicating that even a sub-percent-level bias in modeling the
foreground is sufficient to impede any attempt to recover the 21 cm signals
(e.g., Beardsley et al. 2016 ApJ??; ??; ??
% 【最好引用MWA、PAPER等实测、且谈及前景的文献】).

A variety of statistical and blind source separation approaches have been
proposed to remove one or more foreground components that contaminate the
EoR signals, and they can be classified into two broad categories:
foreground removal (ref??) and foreground avoidance (ref??). Foreground
removal refers to the approaches with which the foreground components are
directly separated from EoR signals by applying appropriate foreground
models. Foreground avoidance, on the other hand, offers an opportunity to
recover a significant part of the EoR signals by abandoning a wedge-shaped
region in the Fourier $k_{\bot} - k_{\parallel}$ space (i.e., cylindrical
power spectrum; The subscript $\bot$ denotes the direction along the line of
sight, and $\parallel$ the direction perpendicular to $\bot$), where the
contamination of the spectrally smooth foreground signals dominate the data
only on small $k_{\parallel}$ scales
% 【视场的问题-大k_//时三角形形状的由来-是否也请用简练词语补上】.
In the latter case the region free of foreground is
defined as the EoR window, which has been explored recently both by
simulations (e.g., ref??) and by analyzing actual data acquired with, e.g.,
the PAPER1 (Ali et al. 2015) and MWA (Paul et al. 2016)
% 【近期是否还有其它观测??】.

Note, however, that there are advantages and disadvantages for both
approaches (e.g., Pober et al. 2013; Chapman et al. 2016; Jensen et al.
2016). For example, foreground removal approaches keep complete information
of EoR in the study, but have to minimize the risk of suffering severe
foreground residuals. Foreground aoidance approaches, on the other hand,
deal with the data free of foreground, but lose all of the modes outside the
EoR window. Also, foreground avoidance strongly depends on the assumption
that the wedge-shaped region can be well-defined, i.e., no significant
foreground signals leak into the window.

In this paper we attempt to investigate the effects of the contamination
contributed by galaxy clusters in foreground avoidance, 【and discuss how
they can be removed safely in upcoming SKA experiment是否可能谈及??】. Among
various celestial foreground components, the cluster component is relatively
less discussed, although it may contribution about ??\% of the extragalactic
foreground power in $50 - 200$ MHz, where the EoR signals are expected to
appear. As of today, limited by the insufficient sensitivity and the
relatively small sample (about ?? diffuse giant/mini radio halos and ??
enlongated radio relics), the origin of the low frequency radio emission of
galaxy clusters, mostly in the form of synchrotron emission, is still poorly
understood. The task of correctly modeling the radio emission of galaxy
clusters becomes more challenging when we extend our consideration to either
the faint end or higher redshifts ($1 < z < 3$??). Obviously, if the
cumulative contribution of both the low-flux cluster sources and the
residuals caused by missubtraction of the brightest clusters, which is
estimated to be about/at least 2?? orders of magnitude above the EoR
signals,
is not properly treated, it will interact with instrumental effects and
leave intangible noise-like contamination in foreground avoidance
(i.e., mode mixing) as an impediment to detection of EoR signals (e.g.,
Datta et al. 2010; Pober 2013 PAPER results文; Beardsley et al. 2016 MWA初步
结果文; Morales 2012; ).

This paper is organized as follows. In \S?? we describe the details of our
new software name?? v??【加注脚注；网址】, which has been developed to
simulate the low frequency sky based on the works of Wang et al. (2010,
2013??). Comparing with our previous works, the simulations of the Galactic,
extragalactic point source, and ?? components have been improved. In
particular, we have significantly enhanced the modeling of galaxy clusters
by applying ?? and ??new model/theory/algorithm??. In \S?? we model a
$10^{\circ} \times 10^{\circ}$ deep sky region in $?? - ??$ MHz, which is
analogous to those be observed with SKA1-low (see ref?? for updated SKA
observing strategy), with oursoftwarename?? v??, and incorporate the
obtained sky model with the SKA instrumental responses available at ?? into
the OSKAR software v?? (ref??; 同时脚注网址) to simulate visibility data
sets. Then we create sky images using CASA v?? (ref??), construct the
residual dirty maps by applying the name?? cleaning algorithm (ref??), and
calculate cylindrical power spectra via 3D Fourier transform of the dirty
maps. In \S?? we quantitatively examine to what extent the EoR window is
contaminated by the residuals contributed by missubtracted and un-subtracted
galaxy clusters.

【 至 17-08-19 17:20 】

\noindent\dotfill~\XU{[END]}~\dotfill
...
\XU{[引言之中不能如此详细列表]}
\begin{itemize}
  \item bright: \numrange{\sim 2}{3} orders of magnitude stronger
    than the EoR signals;
  \item extended: relatively large angular sizes, may extend to several tens
    of arcminutes, corrupting the underlying EoR signal, which appears
    at similar scales;
...
We also make use of the Numba\footnote{Numba: A compiler for Python
  array and numerical functions; \url{http://numba.pydata.org/}}
just-in-time compiler to optimize the overall foreground simulation
speed of the our software.

\LI{[New]} \TODO{[Needs improvements!]}
According to the current published specifications \citep{dewdney2015ska},
the SKA1-Low interferometer consists of antenna stations of \SI{35}{\meter}
in diameter, and has baselines up to \SI{\sim 65}{\kilo\meter},
which corresponds to a FoV of \SI[product-units=power]{\sim 4 x 4}{\deg}
and angular resolution of \SI{\sim 7}{\arcsec} at \SI{158}{\MHz}.
In this work, we simulate the foreground emissions within a sky
patch of \SI[product-units=power]{10 x 10}{\deg} centered at
(R.A., Dec.) = (\SI{0}{\degree}, \SI{-27}{\degree}), the ``EoR0'' field
identified by the MWA \citep{beardsley2016}.
...
\NOTE{I have asked G. Giardino for the all-sky spectral index map
  they derived using the 408, 1420, and 2326 MHz surveys, which I think
  is more appropriate for the low frequencies than the  PSM's one.
  Already got a basic reply, waiting for her sending me the spectral
  index map in about a week ...}
...
\NOTE{连: (1) 请补充一下你的研究内容和目标;
  (2) 请检查此小节内容是否有正确; (3) 请帮忙适当增强/改进. 谢谢.}
...
We create the ``observed'' light-cone cube from the simulated coeval cubes
following the procedure outlined by \citet{datta2012}.
