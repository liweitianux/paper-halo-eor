...
In this work, we employ the Press-Schechter formalism, turbulence
re-acceleration model, and the latest SKA1-Low layout configuration to
simulate the SKA \enquote{observed} images of radio halos.
We calculate the one-dimensional power spectra from simulated images and
find that radio halos can be about \numlist{e4;e3;e2} times more luminous
...
halos to the EoR signal on scales of
$\SI{0.5}{\per\Mpc} \lesssim k \lesssim \SI{1}{\per\Mpc}$ can still be
about \numrange{10}{20}, \numrange{0.3}{1}, and \numrange{0.1}{0.8} in
the three bands, when the \SI{68}{\percent} errors caused by the
uncertainties of the number density of bright radio halos are considered.
In addition, we find that radio halos located inside the far side-lobes
...
\citep[e.g.,][]{cassano2005,donnert2014}
\begin{equation}
...
Moreover, the total energy of the injected electrons can be assumed to
account for a fraction ($\eta_e$) of the ICM's total thermal energy
...
in the beginning of the first merger and is approximated as the
starting time of the first merger.

%........................................................................
\ssssec{\editwip{Stripping Radius}}
...
and $c_s = \sqrt{\gamma_{\R{gas}} k_B T_{\R{cl}} / (\mu m_u)}$ is the sound
...
however, the period $\tau_{\R{turb}}$ during which the turbulence is
intense enough to be able to effectively accelerate particles is
relatively short: $\tau_{\R{turb}} \simeq 2 L_{\R{turb}} / v_{\R{imp}}$,
where $v_{\R{imp}}$ is the relative impact velocity between the merging
clusters \citep{miniati2015}.
Starting from a sufficiently large distance with zero velocity,
the relative impact velocity of two merging clusters with masses
...
$\left( M^{(i)}_{\R{vir,m}}, M^{(i)}_{\R{vir,s}}, t^{(i)}_{\R{merger}} \right)$
...
duration $\tau^{(i)}_{\R{turb}}$.
We then solve \autoref{eq:fokkerplanck} by taking into account both
turbulent acceleration and energy losses during the period
$\left[ t^{(i)}_{\R{merger}}, t^{(i)}_{\R{merger}}+\tau^{(i)}_{\R{turb}} \right]$,
while there is no turbulent acceleration at other times.

%........................................................................
\ssssec{Energy Losses}

Among the several mechanisms through which the relativistic electrons
...
with $K_{5/3}(x)$ being the modified Bessel function of 5/3 order,
$\nu_c = (3/2) \,\gamma^2 \nu_L \sin\theta
= 3 e B \gamma^2 \sin\theta / (4\pi m_e c)$
the electron's critical frequency.
The radio halo is assumed to be the same size as the turbulent
region \citep{vazza2011}, i.e.,
$r_{\R{halo}} \simeq L_{\R{turb}} \simeq r_{\R{vir}}/3$,
therefore its radio power at a certain frequency is
\begin{equation}
  \label{eq:halo-power}
  P(\nu) = \frac{4\pi}{3} r_{\R{halo}}^3 J_{\R{syn}}(\nu),
\end{equation}
and its flux density at the same frequency is
\begin{equation}
  \label{eq:halo-flux}
...
tune them in order to achieve good agreement with observations.} % editone
...
Radio halos (100 simulations) &
  \editone{$\left(4.21_{-2.46}^{+7.38}\right) \times 10^3$} &
  \editone{$\left(1.81_{-1.07}^{+3.52}\right) \times 10^3$} &
  \editone{$\left(0.85_{-0.51}^{+1.83}\right) \times 10^3$} \\
...
\begin{figure*}
  \centering
  \includegraphics[width=0.325\textwidth]{gsyn-f158-heat}
  \includegraphics[width=0.325\textwidth]{gff-f158-heat}
  \includegraphics[width=0.325\textwidth]{pointsources-f158-heat-log}
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{21cm-f158-heat2}
...
\tablenotetext{n}{Extrapolated from \SI{610}{\MHz} with spectral index $\alpha=1.4$.}
\tablenotetext{o}{Extrapolated from \SI{610}{\MHz} with spectral index $\alpha=1.3$.}
