...
Radio halos (100 simulations) &
  \editwip{$\left(4.21_{-2.60}^{+11.2}\right) \times 10^3$} &
  \editwip{$\left(1.81_{-1.13}^{+5.28}\right) \times 10^3$} &
  \editwip{$\left(0.85_{-0.54}^{+2.74}\right) \times 10^3$} \\
...
in the \numrange{120}{128}, \numrange{154}{162}, and \numrange{192}{200}
\si{\MHz} bands, respectively.
However, the power of radio halos leaked into the EoR window can be
...
sources can greatly help obtain a EoR window that is not only sufficiently
clean but also as large as possible to preseve maximum information of the
EoR signal.
}
