...
\savesymbol{tablenum}  % workaround symbol confliction
...
%
\newcommand{\NOTE}[1]{\textcolor{olive}{\textbf{NOTE:}}~\uwave{#1}}
...
major foreground contamination components lacks enough attentions in the
literature,
but we believe that it requires a much more serious treatment:
\begin{itemize}
...
\begin{description}
  ...
  %% switch to a table???
  \item[halo]
    Giant radio halos extending on $\sim \!\si{\Mpc}$ scales;
...
Due to their low surface brightness and steep spectra, radio halos and
relics are difficult to find at middle- or high-frequency ranges
(e.g., \SI{1.4}{\GHz}), and currently only $\sim\,$??? halos and
$\sim\,$??? relics have been found \citep[e.g.,][]{duchesne2017,nuza2017}.
However, in the low-frequency radio band, radio halos and relics become
brighter and easier to be detected.  The large field of view (FoV) of
low-frequency interferometer also helps discover more halos and relics more
efficiently.
Furthermore, owing to its excellent sensitivity, the SKA will possibly
discover more than several thousands of halos and relics, providing us with
invaluable data to understand and solve the puzzles with regard to galaxy
clusters.
The faint radio sky can be very different, and we may have unexpected
findings as we go deeper \citep{padovani2016rev,herreraRuiz2017}.
...
And we implemented the foregrounds simulation technique in the
...
Galactic supernova remnants (SNRs) emission,
and extragalactic point sources emission.
...
\begin{equation}
  ...
  T_b(\bm{r}; \nu) = T_b(\bm{r}; \nu=\SI{408}{\MHz})
    \left( \frac{\nu}{\SI{408}{\MHz}} \right)^{-\alpha(\bm{r})},
\end{equation}
where $T_b(\bm{r}; \nu)$ is the Galactic synchrotron intensity expressed
in brightness temperature at sky position $\bm{r}$ and frequency $\nu$,
and $\alpha(\bm{r}) \sim \numrange{2.7}{3.2}$ is the position-dependent
synchrotron spectral index \citep{davies1996}.
...
and $f_d$ is the effective dust fraction in the line of sight actually
absorbing the \Halpha{}.

Then we can convert the absorption-corrected \Halpha{} intensity
$I_{\Halpha}$ to the free-free emission brightness temperature $T_b$
using the relationship \citet{dickinson2003}
\footnote{Their formula has a superfluous factor of $10^3$.}:
\begin{equation}
  \label{eq:halpha-ff}
  \left[ \frac{T_b}{\si{\kelvin}} \right] = \num{9.068e-3}
  \, a(T_e, \nu) \left[ \frac{\nu}{\si{\GHz}} \right]^{-2.1}
  \left[ \frac{T_e}{\SI{e4}{\kelvin}} \right] 10^{(\SI{290}{\kelvin} / T_e)},
\end{equation}
where $T_e$ is the electron temperature in units of \si{\kelvin},
and the factor $a(T_e, \nu)$ is given by:
\begin{equation}
  a(T_e, \nu) = 0.366 \left[ \frac{\nu}{\si{\GHz}} \right]^{0.1}
  \left[ \frac{T_e}{\si{\kelvin}} \right]^{-0.15}
  \left[ 1.5 \ln \left[ \frac{T_e}{\si{\kelvin}} \right] +
    \ln \left[ \frac{\SI{4.995e-2}{\GHz}}{\nu} \right]\right].
\end{equation}
...
$\tau_{\R{halo}}$ shorter than the timescale $\tau_m$ ($\sim\,$2--3 \si{\Gyr})
...
\begin{equation}
  ...
  \left[ \frac{B}{\si{\uG}} \right] =
    \left[ \frac{B_{\langle {M} \rangle}}{\si{\uG}} \right]
...
\end{equation}
with $n_{\R{th}}$ in unit of [\si{\per\cm\cubed}].
...
\TODO{Whether to include relics???}
...
\TODO{Whether to follow Wang2010???}

\NOTE{Whether move this sub-section before the galaxy clusters???}
