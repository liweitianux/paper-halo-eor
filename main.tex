...
\shorttitle{Contributions of Radio Halos to the Low-frequency Radio Sky}
...
redshifted 21~cm signals, among which there are
...
in detecting the very faint EoR signal, since they may still be
...
at \SI{115}{\MHz} caused by extended radio halos and relics
...
\XU{a. 当年理论家基本认为暗晕 = 全部星系团，不考虑气体（当然有技术原因）;
  b. PS 模型上接早期扰动下接今天，所以它的质量函数约等于星系团的质量函数，
  否则就要初始条件了！}
\LI{PS 理论确实只考虑了暗物质的塌缩，描述的是暗晕的质量分布规律
  （可见《宇宙大尺度结构的形成》）。由于后文需要讨论星系团的 ICM，因此
  按宇宙平均重子物质比例确定 ICM 的质量是合理的。
  这里使用了与王婧颖 2010 一文相同的做法。}
\begin{equation}
...
Using a minimum dark matter halo mass of
$M_{\R{min}} = \SI{e14}{\solarmass}$
and a maximum redshift cut at $z_{\R{max}} = 4$,
we apply \autoref{eq:ps-mass-func} to derive the total number of dark
matter halos in any given sky patch, together with their mass and
redshift distributions.
Based on these distributions, we run Monte Carlo simulations to
generate the dark matter halo sample with random masses
$M_{\R{sim}}$ and redshifts $z_{\R{sim}}$.
By assuming that galaxy clusters have a baryon fraction of
$f_b \simeq \Omega_b/\Omega_m$, their total mass is therefore
given by $M_{\R{sim}}^{\R{cl}} = M_{\R{sim}} / (1 - f_b)$.
\XU{cl 建议改为 total}
...
{\color{magenta}%
In order to resolve mergers with mass change $\Delta M_c \ll M$
during the backward tracing of a dark matter halo, the time step size
$\Delta t$ (i.e., $\Delta\omega$) is required to satisfy that
\citep{lacey1993}
\begin{equation}
  \label{sec:dw-step}
...
\end{equation}
and we adopt an adaptive step size of
$\Delta\omega = \Delta\omega_{\R{max}} / 2$ \citep{randall2002}.} % magenta
At a certain step when $\Delta\omega$ is given, the mass change
$\Delta S$ can be randomly drawn from the following cumulative
probability distribution of sub-halo masses
\begin{equation}
...
we trace the merging history of each dark matter halo using a maximum
...
$M^{\R{cl}} = M / (1 - f_b)$, and we extract the information of
all the merger events associated with the main halo (or main cluster)
to carry out the simulations of radio halos.
...
\XU{不合适！何况 $M^{\R{cl}}$ 是 $M^{\R{tot}}$ 中其次的量！}
...
\textcolor{magenta}{%
When the cluster experiences mergers, the turbulence is generated
throughout the ICM and can accelerate the primary electrons to be highly
relativistic, resulting in the observed diffuse synchrotron radiation
such as radio halos.}
...
{\color{magenta}%
\ssssec{Thermal Number and Energy Densities}

The thermal number density $n_{\R{th}}$ of the ICM can be calculated as
\begin{equation}
  \label{eq:number-density-thermal}
  n_{\R{th}} \simeq \frac{3 f_b M}{4\pi \mu m_u \,r^3_{\R{vir}}},
\end{equation}
where
...
Therefore, the thermal energy density $\epsilon_{\R{th}}$ of the ICM
is given by
\begin{equation}
...
temperature:
$k_BT_{\R{cl}} \simeq k_BT_{\R{vir}} = G M \mu m_u / (2 \,r_{\R{vir}})$.} % magenta
...
%........................................................................
{\color{magenta}%
\ssssec{Initial Condition}

To determine the initial electron spectrum $n_e(\gamma, t_0)$ for solving
\autoref{eq:fokkerplanck}, we evolve the accumulated electron spectrum
...
\textcolor{magenta}{%
The interaction details between the turbulence and the ICM thermal and
...
and $E_{\R{merger}}$ is the merger kinetic energy, which can approximated
as the potential energy to be released from the infalling sub cluster
\begin{equation}
  \label{eq:energy-merger}
  E_{\R{merger}} \simeq \frac{1}{2} f_b M_{\R{sub}} v^2_{\R{vir}}
    \simeq \frac{1}{2} f_b M_{\R{sub}}
    \frac{G M_{\R{main}}}{r^2_{\R{main}}},
\end{equation}
where
$M_{\R{main}}$ and $M_{\R{sub}}$ are the virial masses of the two merging
clusters, and $r_{\R{main}}$ is the virial radius of the main cluster.} % magenta
...
\begin{equation}
  ...
  M_{\R{turb}}^b = f_b M_{\R{merged}}(<L_{\R{turb}})
    = f_b f_{\R{nfw}}(1/3) (M_{\R{main}} + M_{\R{sub}}).
\end{equation}
By substituting \autoref{eq:mass-turb} into \autoref{eq:energy-turb},
we obtain the turbulence velocity dispersion as
\begin{equation}
...
\textcolor{magenta}{%
indicating that the turbulent acceleration is a relatively efficient
mechanism to accelerate the electrons in the ICM and can well explain
the generation of radio halos.}
...
\LI{此处说明我们考虑了一个星系团 3 Gyr 之内的全部并合历史。倾向保留。}
For each merger event
...
$\tau^{(i)}_{\R{turb}}$, and solve the Fokker-Planck equation with
...
and the \SI{1.4}{\GHz} integrated flux function comparison between the
observed and simulated radio halos is shown in
\autoref{fig:halos-simucomp-example}(a),
\textcolor{magenta}{%
where we also plot the \SI{158}{\MHz} integrated flux function for the
simulated radio halos, indicating that radio halos are much brighter in
...
and discussions in \autoref{sec:results}.} \XU{delete this sentence}
...
{\color{magenta}%
Following our previous work \citep{wang2010}, we also simulate several
other foreground components, including the Galactic synchrotron and
free-free emissions, and the extragalactic point sources, in order to
carry out power spectrum comparisons between radio halos and other
foreground components and to better characterize the contributions of
radio halos to the low-frequency radio sky.}
...
\citep[and references therein]{wang2010}.}
...
%========================================================================
\subsection{EoR Signals}
\label{sec:eor-signals}
%========================================================================

The sky maps of the EoR 21~cm signals are created using the 2016
...
a big cube of 1.6 comoving \si{\Gpc} and 1024 cells along each side
\citep{mesinger2016}.}
...
case, and then tile and re-scale them to have the same sky coverage and
...
{\color{magenta}%
Since the Galactic free-free emission is much weaker than the Galactic
synchrotron radiation, and there is not need to distinguish the two
diffuse emissions in this work, we combine them in the observational
simulation and subsequent analyses.
...
signals from the overwhelming foregrounds.}
...
\textcolor{magenta}{%
We then discuss how the large fluctuation of radio halo distributions
across sky regions may cause significant difficulties for the EoR
measurements and how to deal with it.}
In addition to the focused frequency band around \SI{158}{\MHz}, we
investigate the impacts of radio halos at lower and higher bands centered
at \SI{124}{\MHz} and \SI{196}{\MHz}, respectively.
We also discuss how the power spectra may be affected by the instrumental
frequency structures resulting from the calibration errors as well as other
