...
poorly understood.
{\color{cyan}%
In this work, we make use of the Press-Schechter formalism, turbulence
...
We calculate the one-dimensional power spectra of these images and find
that radio halos can be about \numlist{e4;e3;e2} times more luminous than
the EoR signal on scales of $\SI{0.1}{\per\Mpc} < k < \SI{2}{\per\Mpc}$
in the \numrange{120}{128}, \numrange{154}{162}, and \numrange{192}{200}
...
model to derive their low-frequency emission maps with the SKA1-Low's
...
however, the period $\tau_{\R{turb}}$ that the turbulence is intense
enough to be able to effectively accelerate particles is relatively
short: $\tau_{\R{turb}} \simeq 2 L_{\R{turb}} / v_{\R{imp}}$,
...
observation or simulation studies, it is essential to further
...
\begin{figure*}[b]
  ...
  \caption{\label{fig:ps1d-3bands}%
    ...
    corresponding \SI{68}{\percent} error of the power spectra for radio
    halos estimated from the 100 simulation runs.
  }
\end{figure*}
...
the corresponding \SI{68}{\percent} errors in each frequency band.
...
{\color{cyan}%
Meanwhile their diffuse and relatively complicated morphologies make it
...
especially toward to lower frequencies (\SI{\sim 120}{\MHz}).
...
foreground contaminating sources and need careful treatments in future
EoR experiments.
} % cyan
