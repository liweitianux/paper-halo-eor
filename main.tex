...
affected by the instrumental frequency artifacts and the radio halos in
the far side-lobes of the station beam.
...
commonly used in the literature, and perform simulations for a
\SI[product-units=repeat]{10 x 10}{\degree} sky region centered at
(R.A., Dec.)\ = (\SI{0}{\degree}, \SI{-27}{\degree}).
This region\footnote{This region is also targeted by the MWA
  (EoR0 field; e.g., \citealt{beardsley2016}).}
locates at a high galactic latitude ($b = \SI{-78.5}{\degree}$)
and passes through the zenith of the SKA1-Low telescope,
thus it is expected to be appropriate for EoR observations.
...
All the simulation parameters are also listed in \autoref{tab:freq-bands}.
