...
morphologies, the total number is only $\sim\,$1000 \citep{li1991,anderson2017}
...
deep learning and convolutional auto-encoders algorithms, for the
...
just-in-time compiler to optimize overall foreground simulation
speed of the our software.

\LI{[New]} \TODO{Add references and update numbers.}
According to the current published specifications (citations???)
the SKA1-Low interferometer consists of antenna stations of \SI{35}{\meter}
in diameter, and has baselines up to $\sim\,$\SI{65}{\kilo\meter},
which corresponding to a FoV of $\sim\,$\SI{4.5}{\degree} and angular
resolution of $\sim\,$\SI{7}{\arcsec} at $\sim\,$\SI{150}{\MHz}.
The SKA1-Low covers the frequency band of \SIrange{50}{350}{\MHz}
with a channel width of ??.
\TODO{Selected foreground sky patch: EoR0 field by MWA; simulation
FoV, resolution, frequencies, etc...}
...
radio sky and accounts for $\sim\,$70\% of the foreground intensity
at $\sim \!\SI{150}{\MHz}$ \citep{shaver1999}.
It has a power-law spectrum\footnote{We adopt the power-law
  spectrum of form: $S(\nu) \propto \nu^{-\alpha}$.}
with the spectral index $\alpha_{\R{syn}}$ varying as sky positions.

\TODO{Describe how the small-scale fluctuations been added to the
original Haslam 408 MHz map, which has resolution of about 56 arcmin.}
...
\TODO{Introduce that we are also working on a more sophisticated and
...
the Galactic free-free emission map.
...
for galaxy clusters \citep{zandanel2014},
