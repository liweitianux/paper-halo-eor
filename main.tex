...
By calculating the two-dimensional power spectra and avoiding the
...
in the three bands,
and can increase up to be about
\SIrange[range-units=repeat]{1000}{2000}{\percent},
\SIrange[range-units=repeat]{30}{100}{\percent}, and
\SIrange[range-units=repeat]{10}{80}{\percent}
within the typical \SI{68}{\percent} uncertainties due to large
fluctuations of bright radio halos.
Moreover, radio halos inside the far side-lobes of the station beam
can impose strong contamination within the EoR window.
In conclusion, we argue that radio halos are a severe foreground
component to the EoR detection and require serious treatment in the
...
tune them in order to achieve better agreements with the observations.
...
observed one in the high-flux end within simulation uncertainties.
...
brighter in the low-frequency regime.
...
the three bands.
{\color{cyan}%
These results evidently show that radio halos are one of the severe
...
avoidance methods are applied, an EoR window that is free from the
...
in \SIrange{192}{200}{\MHz} (solid lines).
{\color{cyan}%
Within the typical \SI{68}{\percent} error bars (shaded regions), the
power ratios on such scales can increase up to be about
\SIrange[range-units=repeat]{1000}{2000}{\percent},
\SIrange[range-units=repeat]{30}{100}{\percent}, and
\SIrange[range-units=repeat]{10}{80}{\percent} in the three bands.

Based on the above results, we conclude that radio halos are a severe
foreground component to the EoR observations at \SI{\sim 200}{\MHz}
and become a much severer contaminating source at \SI{\sim 120}{\MHz}.
...
significant contamination on the EoR measurements, especially at lower
frequencies (\SI{\sim 120}{\MHz}).} % cyan
...
become much severer, especially inside the EoR window, where most of the
foreground contamination from our Galaxy and extragalactic point sources
is effectively avoided.
The results confirm our conclusion made in \autoref{sec:ps2d} that radio
halos are an important foreground component to the EoR measurements.}
...
calculate the 2D power spectrum.} % cyan
...
SKA1-Low layout configuration.
By making use of the 1D and 2D power spectra, we have carried out
detailed comparisons between radio halos and the EoR signal as well as
...
EoR observations when applying either the foreground removal or 
foreground avoidance methods to measure the EoR signal.
...
for simulating radio halos (\autoref{sec:cluster-halos}) are not
constrained and tuned very well.
...
Moreover, the large-scale deep radio surveys are necessary to identify
the appropriate sky regions for the EoR observations, which is a
challenging task since such regions require as less bright radio
sources in both the main and side lobes as possible.
} % cyan
...
Fred Dulwich for sharing the latest SKA1-Low layout configuration as
