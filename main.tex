...
% XXX/WANG: 此标题看上去像是对射电晕观测的调研综述
\title{\bfseries%
  \color{cyan}%
  Simulation of Radio Halos in Galaxy Clusters and
  Their Contamination on the Epoch of Reionization Observations
}
...
All the simulation parameters are listed in \autoref{tab:freq-bands}.} % cyan
...
\XU{确认这样写参数是否规范} \LI{这种多参数的写法是常用的}
...
central frequencies of the three adopted frequency bands.
...
the observing frequency, i.e., $\fov \propto f^{-1}$, we choose to
keep the central
...
foreground contamination on the EoR measurements.} % cyan
...
\begin{figure*}
  ...
  \caption{\label{fig:ps1d-3bands}%
    ...
    The red solid lines and shaded regions represent the median the
...
and the EoR signal in the three adopted frequency bands are presented
in \autoref{fig:ps1d-3bands}, where we also show the power spectra of
Galactic diffuse emission and extragalactic point sources for reference.
We find that radio halos are generally stronger than the EoR signal by
about 4, 3, and 2 orders of magnitude on all scales
...
about \numrange{2}{4} orders of magnitude stronger than the target
EoR signal in the \SIrange{120}{200}{\MHz} frequency band.
We therefore conclude that radio halos are an important foreground
...
Galactic diffuse emission dominates on the very large scales
...
We are required to use a very conservative EoR window boundary
...
Therefore, we conclude that radio halos can imprint non-negligible
contamination on the EoR measurements at higher frequencies
(\SI{\sim 200}{\MHz}) and become a severe contaminating foreground at
lower frequencies (\SI{\sim 120}{\MHz}), even inside the EoR window
where the strong foreground contamination is effectively avoided.
...
by radio halos in these two situations.} % cyan
...
We thus do not present the relevant figures in the paper.
...
troublesome contaminating source, especially inside the EoR window, to
the EoR measurements.
...
We remove the radio halos with an angular distance of \SI{< 10}{\degree}
from the center, and obtain the simulated observations using the
\texttt{OSKAR} simulator.
...
weighting with a robustness of zero,
and the central \SI[product-units=repeat]{5 x 5}{\degree} regions are
cropped out to calculate the 2D power spectrum,
...
white solid line, and sacrifices more information of the EoR signal.
...
The conclusion drawn from the results is that radio halos are a serious
