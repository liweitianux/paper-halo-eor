...
about
\editone{%
\SIrange{100}{350}{\percent},
\SIrange{8}{43}{\percent}, and
\SIrange{3}{18}{\percent}}
...
\editwip{%
One remaining parameter is the radius $r_{\R{turb}}$ of the turbulence
region, which is estimated to be
\begin{equation}
...
\citep{vazza2012}.
However, the generated turbulence by a minor merger is weak because the
injected energy $E_m$ is much smaller than a major one
...
\begin{figure*}
  ...
  \caption{\label{fig:spec-evo}\editone{%
    ...
    corresponding reference spectra (dashed lines; see
...
$t^{(i)}_{\R{end}} = t^{(i)}_{\R{begin}}+\tau^{(i)}_{\R{turb}}$ is the
time when this merger becomes inactive.
At other times when there is no active merger, only the base
turbulence contributes to the acceleration of electrons, which, however,
is insufficient to balance the energy loss due to synchrotron radiation and
...
spectrum to the volume of radius $R_{\R{turb}}$ for mergers with a smaller
turbulence region.
...
%------------------------------------------------------------------------
\subsubsection{\editone{Halo \editwip{Identification} and Size}}
\label{sec:halo-size}
%------------------------------------------------------------------------
...
exists from about 10.6 to 11.0 \si{\Gyr} at \SI{1.4}{\GHz} and
...
the observed spectral index is obtained from two separated frequencies
(e.g., 0.3 and 1.4 GHz; \citealt{feretti2012rev}).
...
it is necessary to tune them to force the model predictions (e.g., the halo
flux function, the scaling relation between the halo power and the hosting
cluster mass) to be consistent with observations.} % editone
...
\begin{figure}
  ...
  \caption{\label{fig:halo-power}\editone{%
    ...
    and dashed red line shows the fitted relation of
