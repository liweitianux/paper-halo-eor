...
\title{\bfseries%
  \color{cyan}%
  Simulating Radio Halos and
  Evaluating Their Contamination on the Epoch of Reionization Observations
}
...
are less studied and their impacts on the EoR detection are still poorly
known.
...
The comparison of the one-dimensional power spectra shows that radio
halos can be generally about \numlist{e4;e3;e2} times stronger than the
EoR signal in the \numrange{120}{128}, \numrange{154}{162}, and
\numrange{192}{200} \si{\MHz} bands, respectively.
By calculating the two-dimensional power spectra and avoiding the severe
foreground wedge, the power ratios of radio halos to the EoR signal can
still be about \SIrange[range-units=repeat]{30}{100}{\percent},
...
uncertainties due to large distribution variation of radio halos across
the sky.
Moreover, radio halos inside the far side-lobes of the station beam
can impose strong contamination within the EoR window.
In conclusion, we argue that radio halos are an important foreground
contamination to the EoR detection and require serious treatment in the
...
which is expected to be ideal for EoR observations.
...
\tablecaption{\label{tab:freq-bands}%
  Simulation Parameters for the 3 Frequency Bands
}
\tablehead{
...
Region coordinate &
  \multicolumn{3}{c}{R.A.\ = \SI{0}{\degree}, Dec.\ = \SI{-27}{\degree}} \\
Region size &
...
galaxy clusters in any given sky patch, together with the mass and
redshift distributions.
Then galaxy cluster sample is then built by randomly drawing mass and
...
$n_e'(\gamma) = Q_e(\gamma) \,\tau_0$ for \SI{\sim 1}{\Gyr} by applying
...
one in the high-flux end within simulation uncertainties.
...
\begin{figure}
  ...
  \caption{\label{fig:halos-simucomp}%
    The \SI{1.4}{\GHz} integrated flux function comparison between the
    observed (purple dashed line) and simulated (purple solid line)
    radio halos.
    We also plot the \SI{158}{\MHz} integrated flux function
    for the simulated halos (blue solid line), which shows that radio
    halos are much brighter at the low-frequency band.
...
{\color{cyan}%
Since the number density of massive galaxy clusters, with which the
brightest radio halos are generally associated, can vary significantly
across the sky, we repeat the simulation of radio halos for 100 times.}
...
\tablecaption{\label{tab:tb-rms}%
  The \rms{} Brightness Temperatures of the Foregrounds and the EoR Signal
  (Unit: \si{\mK})
}
\tablehead{
  \colhead{Component} &
...
redshifted EoR signal among \SIrange{120}{200}{\MHz}
($z = \numrange{6.1}{10.8}$).
The corresponding \rms{} brightness temperatures at the central
frequencies of the three adopted frequency bands and the map of the EoR
signal obtained at \SI{158}{\MHz} are given in \autoref{tab:tb-rms} and
...
\begin{figure}
  ...
  \caption{\label{fig:eor-tbrms}%
    The \rms{} brightness temperatures of the EoR signal (green solid
    line) within \SIrange{120}{200}{\MHz} (i.e., redshift range of
    \numrange{6.1}{10.8}).
...
halos on the EoR detection, it is essential to take account of the
...
because the marginal regions suffer from the problem of inadequate CLEAN.}
...
we assume that the brightest ones with a \SI{158}{\MHz} flux density
...
at \numlist{124;158;196} \si{\MHz}, respectively.
Moreover, we create foreground image cubes in each frequency band
...
{\color{cyan}%
To suppress the significant side-lobes in the Fourier transform results
caused by the sharp discontinuities at the ends of the finite frequency
band,
\LI{这里的 side-lobes 不会产生混淆；无替代术语可用；其实，
望远镜/干涉阵的波束的旁瓣成因与此类似，即只能测得有限范围的空间/频谱信息。}
we apply the Blackman-Nuttall window function to the frequency
dimension before calculating the power spectra
\citep[e.g.,][]{trott2015,chapman2016}.} % cyan
...
and measure the EoR signal only inside the EoR window to avoid the
...
illustrates the severe contamination caused by radio halos in the EoR
measurements with the foreground removal methods.
...
generally stronger than the EoR signal by up to about 4, 3, and 2 orders
of magnitude on all scales ($\SI{0.1}{\per\Mpc} < k < \SI{2}{\per\Mpc}$)
in the \numrange{120}{128}, \numrange{154}{162}, and \numrange{192}{200}
\si{\MHz} bands, respectively.
Given the large variations (e.g., brightness, number density) of radio
halos across the sky, their power spectra can vary about \numrange{10}{100}
times with respect to the median ones within the typical \SI{68}{\percent}
uncertainties (red shaded regions).
We also find that on large scales ($k \lesssim \SI{0.1}{\per\Mpc}$) the
Galactic foreground is the strongest contaminating source, but its
intensity deceases rapidly as the scale becomes smaller and is weaker
than the median intensity of radio halos by a factor of about
\numrange{10}{100} on scales of
$\SI{0.5}{\per\Mpc} \lesssim k \lesssim \SI{1}{\per\Mpc}$
in the three bands.
The results evidently show that radio halos, which can be generally
about \numrange{2}{4} orders of magnitude stronger than the EoR signal
in the \SIrange{120}{200}{\MHz} frequency band, are one of the significant
foreground components, and they must be carefully modeled and subtracted
when employing the foreground removal methods to extract the EoR signal.
} % cyan
...
illustrating rapid fluctuations along the line-of-sight dimension,
while the spectral-smooth foreground components dominate only in the
low-\klos{} regions ($\klos{} \lesssim \SI{0.2}{\per\Mpc}$).
{\color{cyan}%
With regard to the angular dimension,
the power of radio halos appears on a large range of scales of
$\kperp \lesssim \SI{1}{\per\Mpc}$ but tends to concentrate on the
intermediate scales of $\kperp \sim \SI{0.5}{\per\Mpc}$.
Meanwhile, the powers of Galactic diffuse emission and extragalactic
point sources dominate on the very large scales of
$\kperp \lesssim \SI{0.1}{\per\Mpc}$ and a broad angular scales of
$\kperp \gtrsim \SI{0.1}{\per\Mpc}$, respectively.} % cyan
...
$\gtrsim \SI{0.3}{\per\Mpc}$, and $\gtrsim \SI{0.5}{\per\Mpc}$ in
\numrange{120}{128}, \numrange{154}{162}, and \numrange{192}{200}
\si{\MHz} bands, respectively.
It is also clearly shown that radio halos turn to cause severer
...
evidently shown in Figures~\ref{fig:ps2d} and \ref{fig:ps2d-ratio}
for the 2D power spectra of the foreground components.
To apply the foreground avoidance methods, an EoR window that is free
from the foreground contamination must be defined in the
$(\kperp, \klos)$ plane.
{\color{cyan}%
We have tested multiple parameter configurations of ($e$, $\Theta$) as
defined in \autoref{eq:eor-window}, and have to use $e = 3$ and $\Theta$
of about 3 times the radius of SKA1-Low's \fov{} (i.e.,
$\Theta = \SI{7.5}{\degree}$, \SI{6.0}{\degree}, and \SI{4.8}{\degree}
in the \numrange{120}{128}, \numrange{154}{162}, and \numrange{192}{200}
\si{\MHz} bands, respectively),
which define a conservative EoR window boundary, to effectively avoid
the contaminating wedge.
As shown in Figures~\ref{fig:ps2d} and \ref{fig:ps2d-ratio},
although the relatively clean EoR windows can be defined by using the
chosen parameters, a significant part (about \SI{55}{\percent},
\SI{54}{\percent}, and \SI{40}{\percent} in the \numrange{120}{128},
\numrange{154}{162}, and \numrange{192}{200} \si{\MHz} bands,
respectively) of the power of the EoR signal is lost.} % cyan
...
\begin{figure}
  ...
  \caption{\label{fig:ps1d-ratio}%
    ...
    by averaging the modes only inside the EoR window.
...
avoidance methods are applied, we average the modes only inside the EoR
window to obtain the 1D power spectra, and calculate the 1D power ratios
$R_{\R{eorw}}(k)$ of radio halos to the EoR signal.
We find that, on the scales of
$\SI{0.5}{\per\Mpc} \lesssim k \lesssim \SI{1.0}{\per\Mpc}$,
the 1D power ratio is generally about
\SIrange[range-units=repeat]{30}{100}{\percent} in \SIrange{120}{128}{\MHz},
\SIrange[range-units=repeat]{1}{10}{\percent} in \SIrange{154}{162}{\MHz},
and \SIrange[range-units=repeat]{0.3}{2.5}{\percent}
in \SIrange{192}{200}{\MHz} (\autoref{fig:ps1d-ratio}, the solid lines).
{\color{cyan}%
The power ratios on such scales can increase by about \numrange{10}{20}
times with respect to the above median values within the typical
\SI{68}{\percent} error bars (the shaded regions).

Therefore, we conclude that radio halos can imprint moderate
contamination on the EoR measurements at higher frequencies
(\SI{\sim 200}{\MHz}) and become a severe contaminating source at
lower frequencies (\SI{\sim 120}{\MHz}), even inside the EoR window
where a large port of the strong foreground contamination is
effectively avoided.
The large fluctuation of the power spectra of radio halos suggests
that we should choose a sky region with as least bright radio halos
as possible to effectively avoid the contamination.
We will discuss the EoR region selection further in
\autoref{sec:far-sidelobes}.
} % cyan
...
frequency artifacts in the reduced image cubes;
foreground sources located in the side-lobes of the station beam can
significantly reduce the imaging dynamical range and quality.
...
We therefore investigate two extreme cases here:
...
{\color{cyan}%
we calculate the
2D power spectrum ratios $R_{\R{arti}}(\kperp, \klos)$ of the image cube
with added frequency artifact to the original one,}
...
{\color{cyan}%
The results confirm that radio halos are an important foreground
component to the EoR measurements, and their contamination becomes much
severer, especially inside the EoR window, when even very minor
(\SI{\sim 0.1}{\percent}) instrumental or calibration uncertainties
exist in frequency channels.
} % cyan
...
\begin{figure*}
  ...
  \caption{\label{fig:ps2d-fscn}%
    \textbf{(a)} The \SIrange{154}{162}{\MHz} 2D power spectrum of radio
    halos located in the far side-lobes.
...
{\color{cyan}%
To investigate the impacts of the FSCN contributed by the radio halos
located in the far side-lobes of the station beam, we have to generate
...
We simulate radio halos in the \SIrange{154}{162}{\MHz} band that cover
the sky from the edge of the second side-lobe
($\phi \sim \SI{10}{\degree}$ from the field center) until the horizon
($\phi = \SI{90}{\degree}$) for the telescope.
This emulates an ideal CLEAN procedure in practical data analysis that
removes all the radio halos from the main lobe and the first side-lobe,
leaving the ones only in the far side-lobes.
We then obtain the dirty images of the central
\SI[product-units=repeat]{5 x 5}{\degree} region using the
\texttt{OSKAR} simulator and the \texttt{WSClean} imager as described
in \autoref{sec:obs-simu}.} % cyan

From the calculated 2D power spectrum shown in \autoref{fig:ps2d-fscn}(a),
...
{\color{cyan}%
Consequently,
the existence of severe FSCN contamination makes the EoR region selection
a more challenging task, which requires that as least bright sources as
possible should enter both the main lobe and the side-lobes during the
EoR observations.
In addition, a highly accurate foreground model with various carefully
modeled sources enables us to mitigate the FSCN and is hence of great
importance to the EoR experiments.} % cyan
...
The contamination resulted from the residual frequency artifacts in the
image cubes and radio halos inside the far side-lobes of the station beam
was also investigated.
We conclude that radio halos are a serious foreground component to the
EoR observations regardless of using either the foreground removal or
foreground avoidance methods to extract the EoR signal.
...
us with a unique opportunity to fully explore radio halos in the
