...
\begin{equation}
  \label{sec:dw-step}
  (\Delta\omega)_{\R{step}} \simeq \frac{1}{2} \sqrt{
...
\begin{equation}
  ...
  \right. \quad [\R{Gyr}],
\end{equation}
where $\beta_{\R{turb}} \lesssim 1$ is a free parameter describing the
intensity of the turbulence acceleration
(we assume $\beta_{\R{turb}} = 0.3$ (???) in this work),
...
\begin{equation}
  \label{sec:jnu-sync}
  J_{\R{syn}}(\nu, t_0) = \frac{\sqrt{3}e^3 B}{c}
  \int_{\gamma_{\R{min}}}^{\gamma_{\R{max}}} \int_0^{\pi/2}
  \sin^2 \!\theta \,n(\gamma, t_0) F\!\left( \frac{\nu}{\nu_c} \right)
  \,\D{\theta} \,\D{\gamma},
\end{equation}
...
and $\nu_L = e B / (2\pi m_e c)$ the Larmor frequency.
...
\begin{equation}
  \label{eq:rho-crit}
  \rho_{\R{crit}}(z) = \frac{3 H(z)^2}{8 \pi G},
\end{equation}
