...
{\color{magenta}(several to tens of thousand in all the sky)}
...
we choose $\Delta M_c = \SI{e13}{\solarmass}$
}
\LI{无具体文献可引；测试显示该参数对模拟结果影响很小}.
...
$\eta_e = \SI{0.1}{\percent}$, $s = 2.3$, $\zeta = 0.1$,
...
and run the radio halos simulation for 100 times, given the fact that
radio halos vary between different sky regions.
...
\begin{figure*}
  \gridline{
    \fig{fluxfunc-simucomp-1400}{0.49\textwidth}{(a)}
    \fig{halos-f158-C17}{0.45\textwidth}{(b)}
  }
...
We divide the chosen \SIrange{154}{162}{\MHz} frequency segment into 51
channels with a frequency resolution of \SI{160}{\kilo\hertz}.
{\color{magenta}
At every frequency channel, we simulate the input sky maps as described
in \autoref{sec:sky-simu},}
...
Using the simulated image cubes, we can now calculate the power spectra
for each component, from which we can quantitatively evaluate the
contamination to the EoR detection contributed by radio halos.
...
We also discuss the requirement of radio halos subtraction for a robust
EoR detection.

%========================================================================
\subsection{Power Spectra and EoR Window}
\label{sec:ps-eor}
%========================================================================
...
do not have sufficient sensitivity to directly image the structures of
the signal, thus statistically probing the signal by measuring its power
spectra is the more feasible and promising way.
The redshifted 21~cm EoR signal as observed at different frequencies
represents a fully 3D data cube, where the two angular dimensions describe
the transverse distances on the sky and the spectral dimension
characterizes the line-of-sight distance to the \Hi{} sources.
...
\begin{figure*}
  ...
  \caption{\label{fig:ps2d}%
    ...
    The magenta dashed line marks the boundary between the EoR window
    (on the top left) and the contaminating foreground wedge (the bottom
    right).
  }
\end{figure*}
...
In this work, we choose $\Theta = \SI{6}{\degree}$ (corresponding to
about 3 times of the SKA1-Low's FoV at \SI{158}{\MHz}) and $e = 3$
for a conservative EoR window boundary.
...
Moreover, radio halos have significant powers beyond the boundary and
contaminate the 21~cm signal inside the EoR window.
\TODO{test CLEAN with lower threshold and multi-frequency CLEAN to confirm
the results, then explain the radio halos' power extending to high-\klos{} ...}
...
\begin{figure*}
  ...
  \caption{\label{fig:ps2d-ratio}%
    ...
    the EoR signal \textbf{(a)},
    the diffuse Galactic emission \textbf{(b)},
    and the extragalactic point sources \textbf{(c)}.
    All sub-figures share the same colorbar with logarithmic scale.
    The silver dashed line marks the same EoR window boundary as in
    \autoref{fig:ps2d}.
  }
\end{figure*}
...
and is well restricted below the chosen EoR window boundary.
Nevertheless, some power of radio halos still leaks into the EoR window
as similarly shown in \autoref{fig:ps2d}(b),
and the leaked power may be as strong as the EoR signal at
$\SI{0.2}{\per\Mpc} \leq \kperp \leq \SI{1.0}{\per\Mpc}$,
...
stronger contamination at large-\kperp{} ($> \SI{0.2}{\per\Mpc}$) modes.
...
percent of the power of point sources, hence imprint a non-negligible
contamination to the EoR window.
