...
\title{%
  Radio Halos in Galaxy Clusters and
  Their Contributions to the Low-Frequency Radio Sky
}
%\title{%
...
to probing the Epoch of Reionization (EoR) through measuring the redshifted
21~cm signal.
A foreground model as accurate as possible is very valuable and requires
that we understand and characterize all the major and minor foreground
components.
This work focuses on radio halos in galaxy clusters, a diffuse foreground
that is not well studied and less constrained by observations.
We employ the merger-induced turbulence re-acceleration model to simulate
sky images of radio halos, and generate the \enquote{observed} images by
adopting the latest SKA1-Low layout configuration to incorporate the
practical instrumental effects.
We perform detailed comparisons of power spectra between radio halos and
the EoR signal in three frequency bands (\SIrange{120}{128}{\MHz},
\SIrange{154}{162}{\MHz}, and \SIrange{192}{200}{\MHz}).
Although we calculate the one-dimensional power spectra with a
conservative EoR window applied to avoid most of the contamination,
the power ratio of radio halos to the EoR signal is typically about
\SIrange[range-units=repeat]{1}{10}{\percent} on scales of about
\SIrange{0.2}{1.0}{\per\Mpc} in the \SIrange{154}{162}{\MHz} band, and
can be \SIrange[range-units=repeat]{10}{100}{\percent} when accounting
for the distribution fluctuation of radio halos across the sky.
We also find that radio halos impose more contamination toward lower
frequencies (\SI{\sim 100}{\MHz}).
Therefore, we conclude that radio halos need more serious treatment for
the forthcoming deep EoR observations, and identifying clean sky
regions is advantageous in obtaining the best observation results and
making the best of instruments.
}
% ~237 words
\end{abstract}
...
will cover the first side-lobe of the station beam.
...
For each galaxy cluster, we first employ the extended Press-Schechter theory
to simulate its merging history, which is described by a merger tree.
...
\XU{确认这样写参数是否规范}
of a galaxy cluster simulated in \autoref{sec:merging-history}, where
$t^{(i)}_{\R{merger}}$ denotes the starting time of the $i$-th merger
...
into account both turbulent acceleration and energy losses during the
...
The mean (r.m.s.) brightness temperature of the 100 simulations at
\SI{158}{\MHz} is
...
\begin{figure*}
  \gridline{
    \fig{fluxfunc-simucomp-1400}{0.51\textwidth}{%
      (a) Integrated flux function comparison}
    \fig{halos-f158-heat-log}{0.45\textwidth}{%
  ...
  \caption{\label{fig:halos-simucomp-example}%
    ...
    the simulated radio halos at \SI{158}{\MHz}, and the color bar is in
    units of \si{\kelvin}.
  }
\end{figure*}
